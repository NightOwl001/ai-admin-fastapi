...
基于 Python + FastAPI 构建的后端管理系统，采用标准三层架构，独立实现用户注册/登录、JWT 身份认证、用户信息增删改查（含软删除）、分页/模糊查询等核心功能。项目重点修复了模糊查询失效、非法用户注册等关键 Bug，并引入软删除机制保障数据可恢复性，通过索引优化提升查询性能，确保系统稳定性与数据安全性。后续计划封装 Function Calling 工具能力对接大模型，探索 RAG 知识库问答模块。
...
```text
ai-admin-fastapi/
├── api/            # 接口控制层：路由定义（user_router.py）
├── service/        # 业务逻辑层：核心功能处理（user_service.py）
├── dao/            # 数据访问层：数据库操作封装（user_dao.py）
├── model/          # 数据模型：ORM实体类 + 请求/响应校验（entity.py / schema.py）
├── utils/          # 工具类：JWT、数据库连接、日志、统一响应（jwt_util.py / db.py / log_util.py / response.py）
├── config/         # 项目配置：环境变量读取（settings.py）
├── .env.example    # 环境变量模板
├── README.md       # 项目说明文档
